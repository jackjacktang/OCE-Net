...
"""
class HCP_FOD_norm_NIFTI_interface(Abstract_IO):
    ...
    def initialize(self, input_path):
        ...
        brain_dir = os.path.join(input_path, 'FOD_norm')

        # Identify samples
        brain_name_list = os.listdir(brain_dir)
        sample_list = []
        for brain_name in brain_name_list:
            # Skip if file does not end with nii.gz
            if not brain_name.endswith("_WMfod_norm.nii.gz"): continue
            sample_list.append(brain_name[:-len("_WMfod_norm.nii.gz")])
    ...
    def load_image(self, index):
        ...
        img_path = os.path.join(self.data_directory, 'FOD_norm')
        # print('loading from img_path: ', img_path, str(index) + "_WMfod_norm.nii.gz")
        if not os.path.exists(img_path):
            raise ValueError(
                "Image could not be found \"{}\"".format(img_path)
            )
        # Load volume from NIFTI file
        vol = nib.load(os.path.join(img_path, str(index) + "_WMfod_norm.nii.gz"))
        aff1 = vol.affine
    ...
    def load_segmentation(self, index):
        ...
        seg_path = os.path.join(self.data_directory, 'lesion')
        # print('loading from seg_path: ', seg_path, str(index) + "_lesion.nii.gz")
        if not os.path.exists(seg_path):
            raise ValueError(
                "Lesion could not be found \"{}\"".format(seg_path)
            )
        # Load segmentation from NIFTI file
        seg = nib.load(os.path.join(seg_path, str(index) + "_lesion.nii.gz"))
        # Transform NIFTI object to numpy array
